...
class WP_Widget_Recent_Posts_Plus62 extends WP_Widget {
	/** constructor */
	function WP_Widget_Recent_Posts_Plus62() {
		$widget_ops = array('classname' => 'widget_posts_wrap', 'description' => __( 'Plus62 - Recent Posts', 'plus62') );
		$this->WP_Widget('recent_posts_plus62', __('Plus62 - Recent Posts', 'plus62'), $widget_ops);
	}

	/** @see WP_Widget::widget */
	function widget($args, $instance) {		
		extract( $args );
		$default = array ('widget_title'=>__('Recent Posts','plus62'), 'cats'=>'', 'cat'=>'', 'quantity'=>1, 'exclude'=>'', 'order'=>'date', 'display'=>'type-2', 'excerpt'=>'display', 'not_in'=>'' );
		...
		?>
			...
			while ($r->have_posts()) : $r->the_post();
				if ( $display == 'type-1' || ( $display== 'type-2' && $i == 0) ) { $title_class = 'post-title h2'; $wrap_class = 'type-1'; }
				elseif ( $display== 'type-2' && $i > 0 ) { $title_class = 'post-title-small h3'; $wrap_class = 'type-2'; }
				else { $title_class = 'post-title-small h3'; $wrap_class = 'type-3'; }
			?>
				<article class="<?php echo $wrap_class; ?>">

							<header>
								<?php if ( has_post_thumbnail() && $display == 'type-3' ) { ?>
									<a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>" class="home-thumb alignleft"><?php the_post_thumbnail('post-thumbnail'); ?></a>
								<?php } ?>
								<h2 class="<?php echo $title_class; ?>"><a href="<?php the_permalink() ?>" rel="bookmark" title="<?php the_title_attribute(); ?>"><?php the_title(); ?></a></h2>
							...
							<section class="post_content clearfix">
								<?php if (has_post_thumbnail() && ( $display=='type-1' || ( $display=='type-2' && $i==0 ) ) ): ?>
								<a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>" class="home-thumb alignleft"><?php the_post_thumbnail('thumb-300'); ?></a>
                                <?php endif; ?>
							</section> <!-- end article section -->
							<?php if ( $display == 'type-1' || ( $display== 'type-2' && $i == 0) ) : ?>
                            <footer>
                            	<span class="comment-count"><?php comments_popup_link('+', '1', '%','','OFF'); ?></span>
                            </footer>
							<?php endif; ?>
	...
	function update($new_instance, $old_instance) {				
		...
		$default = array ('widget_title'=>__('Recent Posts','plus62'), 'cats'=>'', 'cat'=>'', 'quantity'=>1, 'exclude'=>'', 'order'=>'date', 'display'=>'type-2');
...
add_action('widgets_init', create_function('', 'return register_widget("WP_Widget_Recent_Posts_Plus62");'));
